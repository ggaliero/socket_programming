...
int main() {
   ...
   sockfd=socket(AF_INET, SOCK_DGRAM, 0);  // Creates an UDP socket connection
   ...
   // Sets the number of bytes to 0 in the socket address
   ...
   while(!loop){
      ...
      if(strcmp(movie,"exit")== 0){ 
       ...
       sended=sendto(sockfd, movie, strlen(movie),0,(struct sockaddr *) &server, sizeof(server));
         ...
         if(sended < 0){
            ...
            rec=recvfrom(sockfd, received, MAX_LEN,0,NULL,NULL);
            received[rec]='\0';
